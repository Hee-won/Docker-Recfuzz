...
function build_with_Recfuzz() {
    ...
    for TARG in "${@:2}"; do
        ...
        build_target $1 $CC $CXX "-g -O2"

        for BUG_NAME in "${str_array[@]:1}"; do
            copy_build_result $1 $BIN_NAME $BUG_NAME "Recfuzz"
        done
    done
