...
from benchmark import SWFTOPHP47_TARGETS, SWFTOPHP48_TARGETS, CXXFILT_TARGETS, OBJDUMP_TARGETS
...
def print_result(outdir, exp_id, targ_list, timeout, iter_cnt, tools): #targ_list = CVE benchmark
    df_dict = {}
...
def main():
    if len(sys.argv) < 3:
        print("Usage: %s <table/figure/target name> <time> <iterations> \"<tool list>\" " % sys.argv[0])
        exit(1)
    ...
    target_list = ""
    tools_to_run = tools = ["Recfuzz"]
    benchmark = target 


    if "swftophp-4.7" in benchmark:
        target_list = [x for (x,y,z,w) in SWFTOPHP47_TARGETS]
    elif "swftophp-4.8" in benchmark:
        target_list = [x for (x,y,z,w) in SWFTOPHP48_TARGETS]      
    elif "cxxfilt" in benchmark:
        target_list = [x for (x,y,z,w) in CXXFILT_TARGETS]
    elif "objdump" in benchmark:
        target_list = [x for (x,y,z,w) in OBJDUMP_TARGETS]
    else:
        print("++++++++++++++++++ [ERROR] parse_result.py : program if +++++++++++++++")
